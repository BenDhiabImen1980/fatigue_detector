...
LOCATION="eastus"   # ✅ TOUT en East US (région autorisée)
FALLBACK_LOCATION="westus2"  # ✅ Fallback vers West US 2
ACR_NAME="fatigdetectorimen"  # Nom fixe et valide
CONTAINER_APP_NAME="fatigue_detector" 
...
#################################
# 2) Resource Group (réutilise l'existant)
#################################
echo "Vérification du groupe de ressources..."

# Récupérer la région du RG existant
EXISTING_RG_LOCATION=$(az group show -n "$RESOURCE_GROUP" --query location -o tsv 2>/dev/null || echo "")

if [ -n "$EXISTING_RG_LOCATION" ]; then
    echo "✅ RG existant trouvé en: $EXISTING_RG_LOCATION"
    LOCATION="$EXISTING_RG_LOCATION"  # Utiliser la même région
    echo "🔄 Alignement: Toutes les ressources seront créées en $LOCATION"
else
    echo "Création du groupe de ressources en $LOCATION..."
    az group create -n "$RESOURCE_GROUP" -l "$LOCATION" >/dev/null
    echo "✅ RG créé: $RESOURCE_GROUP"
fi
...
# Vérifier si ACR existe déjà
if az acr show --name "$ACR_NAME" --resource-group "$RESOURCE_GROUP" >/dev/null 2>&1; then
    echo "✅ ACR existe déjà: $ACR_NAME"
else
    set +e
    az acr create \
